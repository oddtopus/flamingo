...
class dpCalcDialog:
  def __init__(self):
    ...
    self.fluids=['water','vinegar','acetone','HF','HCl','CCCCCCCC','air']
    self.form.comboFluid.addItems(self.fluids)
    ...
    self.form.comboFluid.currentIndexChanged.connect(self.checkState)
    self.form.editTemperature.textChanged.connect(self.checkState)
    self.form.editPressure.textChanged.connect(self.checkState)
    self.form.editFlow.setText('1')
    self.isLiquid=True
    self.Rho=0
    self.Mu=0
    self.form.radioLiquid.hide()
    self.form.radioGas.hide()
    self.checkState()
  def accept(self):
    Dp=0
    elements=list()
  ...
  def checkState(self):
    ...
    try: fluid=Chemical(self.form.comboFluid.currentText(),T=T,P=P)
    except: fluid=Mixture(self.form.comboFluid.currentText(),T=T,P=P)
